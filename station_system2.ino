...
//
//  Press button → POST station_id to server
//              → server returns buses array with route + ETA
//              → display on LCD + play audio
//
//  Server JSON shape:
//  {"buses": [{"bus_id":12345,"route":"...","dist":0.3,
//              "speed":30.0,"eta_min":1,"eta_lbl":"1 min",
//              "last_seen":"22:34:44"}]}
//
//  WIRING NOTES:
...
const char ENDPOINT[] = "/api/station/";
const char BODY[]     = "station_id=5";
...
// ================================================================
//  JSON helpers — work on a flat object string
// ================================================================
...
// ================================================================
//  Extract the first object {...} inside the "buses":[...] array.
//  Input : full server response string
//  Output: fills busObj with the first bus object, returns true on success
// ================================================================
bool extractFirstBus(const char* raw, char* busObj, uint8_t busObjLen) {
  // Find "buses":[
  const char* arr = strstr(raw, "\"buses\":");
  if (!arr) return false;
  arr = strchr(arr, '[');
  if (!arr) return false;
  arr++;  // step past '['

  // Find the opening brace of the first element
  ...
  uint8_t len = (uint8_t)(p - start + 1);
  if (len >= busObjLen) len = busObjLen - 1;
  strncpy(busObj, start, len);
  busObj[len] = '\0';
  return true;
}
...
void dfPlayTrack(uint16_t track) {
  dfSendCmd(0x03, (track >> 8) & 0xFF, track & 0xFF);
}
...
// ================================================================
//  Display one bus object on LCD + play audio
//
//  Expected fields in busObj:
//    bus_id, route, dist, speed, eta_min, eta_lbl, last_seen
// ================================================================
void displayBus(const char* busObj) {
  float dist   = jsonFloat(busObj, "dist");
  int   etaMin = jsonInt  (busObj, "eta_min");

  char route[21];  jsonStr(busObj, "route",   route, sizeof(route));
  char eta[16];    jsonStr(busObj, "eta_lbl", eta,   sizeof(eta));

  Serial.print("route=");    Serial.print(route);
  Serial.print(" dist=");    Serial.print(dist);
  Serial.print(" eta_lbl="); Serial.print(eta);
  Serial.print(" eta_min="); Serial.println(etaMin);

  char row[21];

  // Row 0: route name (truncated to 20 chars)
  lcdRow(0, route[0] ? route : "Unknown Route");
  ...
  // Row 2: distance
  char distStr[10];
  dtostrf(dist, 0, 2, distStr);
  snprintf(row, sizeof(row), "Dist: %s km", distStr);
  lcdRow(2, row);

  // Row 3: blank / padding
  lcdRow(3, "                    ");

  // ── Audio: play track = eta_min; fallback if null / 0 / negative ──
  uint16_t track;
  bool etaNull = jsonIsNull(busObj, "eta_min");

  if (!etaNull && etaMin > 0) {
    track = (uint16_t)etaMin;
    Serial.print("Playing track "); Serial.println(track);
  } else {
    track = FALLBACK_TRACK_INDEX;
    if (etaNull)          Serial.println("eta_min null  → fallback track");
    else if (etaMin == 0) Serial.println("eta_min 0 (arriving) → fallback track");
    else                  Serial.println("eta_min negative → fallback track");
  }
...
// ================================================================
//  HTTP POST
//  Equivalent to: curl -X POST http://ermias21.pythonanywhere.com/api/station/ -d "station_id=5"
// ================================================================
bool doPost() {
  const char* body = BODY;   // "station_id=5"
  ...
  if (!success) {
    ...
    Serial.println("[WARN] HTTP not 200");
    return false;
  }

  // ── Parse: pull first bus object out of the buses array ──
  static char busObj[220];
  if (!extractFirstBus(serverResp, busObj, sizeof(busObj))) {
    lcdRow(0, "No bus data         ");
    lcdRow(1, "                    ");
    lcdRow(2, "                    ");
    lcdRow(3, "Press button retry  ");
    Serial.println("[WARN] Could not extract bus object from response");
    return false;
  }

  Serial.print("[Bus obj] "); Serial.println(busObj);
  displayBus(busObj);
  return true;
}
